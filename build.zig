...
fn addGLFW(exe: *std.build.Step.Compile) void {
    exe.addIncludePath(std.build.LazyPath.relative("external/glfw/include"));
...
fn addImGui(exe: *std.build.Step.Compile) void {
    ...
    exe.addCSourceFile(.{ .file = std.build.LazyPath.relative("src/imgui_unity.cpp"), .flags = &.{}});
    exe.addCSourceFile(.{ .file = std.build.LazyPath.relative("external/cimgui/cimgui.cpp"), .flags = &.{}});
}

fn addSokol(exe: *std.build.Step.Compile) void {
    exe.addIncludePath(std.build.LazyPath.relative("external/sokol"));
    exe.addCSourceFile(.{ .file = std.build.LazyPath.relative("src/sokol.c"), .flags = &.{}});
}
